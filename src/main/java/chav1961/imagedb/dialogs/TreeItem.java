...
public class TreeItem implements FormManager<Object,TreeItem>, Cloneable {
	...
	@LocaleResource(value="treeitem.refs",tooltip="treeitem.refs.tt")
	@Format("30*3s")
	public ReferenceAndComment[]	refs;
	
	public TreeItem(final LoggerFacade logger, final long id, final long parent, final String name, final String comment, final String tags, final ReferenceAndComment[] refs) {
		this.logger = logger;
		this.id = id;
		this.parent = parent;
		this.name = name;
	...
	public String toString() {
		return "TreeItem [id=" + id + ", parent=" + parent + ", name=" + name + ", tags=" + tags + ", comment=" + comment + ", refs=" + (refs == null ? "null" : Arrays.toString(refs)) +"]";
	}
	...
	public TreeItem clone() {
		try{return (TreeItem)super.clone();
			...
			return new TreeItem(logger, id, parent, name, comment, tags, refs);
		}
	}
}